...
def test_manifest_has_required_hacs_fields() -> None:
    ...
    assert manifest["documentation"] == "https://github.com/cash83/cudy-router-ha-control#readme"
    ...
    assert manifest["version"] == "1.4.2b1"
...
def test_readme_links_directly_to_bug_report_issue_form() -> None:
    ...
    assert "https://github.com/cash83/cudy-router-ha-control/issues/new?template=bug_report.yml" in readme
    assert "Home Assistant version" in readme
    assert "Router model and firmware" in readme
    assert "The entity that failed" in readme
