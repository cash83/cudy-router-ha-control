...
def test_switch_and_select_platforms_cover_router_settings() -> None:
    ...
    assert "SETTINGS_REFRESH_DELAY_SECONDS = 8" in switch_source
    assert "_optimistic_value" in switch_source
    assert "self.async_write_ha_state()" in switch_source
    assert "await asyncio.sleep(SETTINGS_REFRESH_DELAY_SECONDS)" in switch_source
    ...
    assert "SETTINGS_REFRESH_DELAY_SECONDS = 8" in select_source
    assert "_optimistic_value" in select_source
    assert "self.async_write_ha_state()" in select_source
    assert "await asyncio.sleep(SETTINGS_REFRESH_DELAY_SECONDS)" in select_source
