...
def test_collect_router_data_adds_wisp_for_lt300_v2(monkeypatch) -> None:
            ...
            <div class="panel panel-primary">
              ...
              <table class="table">
                ...
                <tbody>
                  ...
                  <tr><td>BSSID</td><td>80:AF:CA:5F:AA:C6</td></tr>
                  <tr><td>Channel</td><td>5</td></tr>
                  <tr><td>Width</td><td>HT40</td></tr>
    ...
    assert ("admin/network/wireless/wds", True) in fake_router.requests
...
def test_collect_router_data_reads_wisp_path_style_detail_probe(monkeypatch) -> None:
    """LT300 WISP collection should read the path-style detail endpoint."""
    monkeypatch.setattr(
        router_data,
        "existing_feature",
        lambda device_model, module: module == const.MODULE_WISP,
    ...
    fake_router = _FakeRouter(
        {
            ...
            <div class="panel panel-primary">
              ...
              <table class="table">
                <thead><tr><th>Status</th><th>Connected</th><th></th></tr></thead>
                <tbody>
                  <tr><td>SSID</td><td>Cudy-tom</td></tr>
                  <tr><td>Signal</td><td>-48 dBm</td></tr>
                </tbody>
              </table>
            </div>
            """,
            "admin/network/wireless/wds/status/detail/1/wisp": _fixture_text(
                "wisp",
                "lt300_v3_wisp_path_detail.html",
            ),
            "admin/network/wireless/wds/data": """
            {"wds":"success","ssid":"Cudy-tom","up":true,
             "bssid":"C8:7F:54:BA:1D:92","hidden":0,"proto":"dhcp",
             "txpower":-1,"channel":11,"htbw":"ht20","isolate":0}
            """,
            "admin/network/wireless/wds/config/nomodal/mode/wisp": """
            <form>
              <input type="hidden" name="cbi.cbe.wds-config.1.enabled" value="1" />
    ...
    assert wisp["status"]["value"] == "Connected"
    assert wisp["status"]["attributes"] == {"raw_status": "success", "up": True}
    assert wisp["ssid"]["value"] == "Cudy-tom"
    assert wisp["public_ip"]["value"] == "113.87.80.146"
    assert wisp["signal"]["value"] == -49
    assert wisp["bssid"]["value"] == "C8:7F:54:BA:1D:92"
    assert wisp["channel"]["value"] == 11
    assert wisp["channel_width"]["value"] == "20 MHz"
    assert wisp["protocol"]["value"] == "DHCP"
    assert wisp["transmit_power"]["value"] == -1
    assert wisp["enabled"]["value"] is True
    assert ("admin/network/wireless/wds/status/detail/1/wisp", True) in fake_router.requests
    assert ("admin/network/wireless/wds/status?detail=1", True) in fake_router.requests
    assert ("admin/network/wireless/wds/data", True) in fake_router.requests
    assert ("admin/network/wireless/wds/config/nomodal/mode/wisp", True) in fake_router.requests
...
def test_collect_router_data_keeps_configured_zerotier_disabled_despite_connected_status(monkeypatch) -> None:
    """The explicit VPN enabled form value should win while a tunnel is shutting down."""
    monkeypatch.setattr(
        router_data,
        "existing_feature",
    ...
    fake_router = _FakeRouter(
        {
            ...
            <input type="hidden" name="cbid.vpn.config.enabled" value="0" />
            <select name="cbid.vpn.config._proto">
              <option value="zerotiers" selected="selected">ZeroTier Master</option>
            </select>
            """,
            "admin/network/vpn/zerotier?embedded=&mvpn=": "",
            "admin/network/vpn/zerotier": "",
            "admin/network/vpn/zerotier/status?detail=": """
            <table>
              <tr><td>Status</td><td>Connected</td></tr>
              <tr><td>Protocol</td><td>ZeroTier Master</td></tr>
              <tr><td>Tunnel IP</td><td>10.216.32.238</td></tr>
            </table>
            """,
        }
    )
    ...
    assert data[const.MODULE_VPN_SETTINGS]["enabled"]["value"] is False
    assert data[const.MODULE_VPN]["tunnel_ip"]["value"] == "10.216.32.238"


def test_collect_router_data_uses_zerotier_status_when_enabled_field_is_missing(monkeypatch) -> None:
    """ZeroTier status should fill enabled only when the settings form lacks that field."""
    monkeypatch.setattr(
        router_data,
        "existing_feature",
        lambda device_model, module: module == const.MODULE_VPN,
    ...
    assert data[const.MODULE_VPN_SETTINGS]["enabled"]["value"] is True
    assert data[const.MODULE_VPN]["tunnel_ip"]["value"] == "10.216.32.238"
