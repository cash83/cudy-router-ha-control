...
_VPN_STATUS_PATHS: tuple[str, ...] = (
    ...
    "admin/network/vpn/zerotier/status",
...
_VPN_STATUS_PATH_PROTOCOLS: dict[str, frozenset[str]] = {
    "admin/network/vpn/zerotier/status?detail=": frozenset({"zerotier", "zerotiers"}),
    "admin/network/vpn/zerotier/status": frozenset({"zerotier", "zerotiers"}),
...
_WISP_STATUS_PATHS: tuple[str, ...] = (
    ...
    "admin/network/wireless/wds/status?mode=wisp&detail=1",
    "admin/network/wireless/wds/status?status=",
    "admin/network/wireless/wds?detail=1",
    "admin/network/wireless/wds?detail=",
...
def _vpn_status_indicates_connected(input_html: str) -> bool:
    """Return whether a VPN status page reports an active tunnel."""
    text = BeautifulSoup(input_html or "", "html.parser").get_text(" ", strip=True).lower()
    if not text:
        return False
    ...
    if any(marker in text for marker in disconnected_markers):
        return False
    return any(marker in text for marker in {"connected", "online", "success", "up"})
...
async def collect_router_data(
    ...
    if existing_feature(device_model, MODULE_VPN) is True:
        ...
        if vpn_settings_html:
            ...
            protocol_value = _entry_value(vpn_settings, "protocol")
            if protocol_value in {"zerotier", "zerotiers"}:
                for zerotier_settings_path in (
                    "admin/network/vpn/zerotier?embedded=&mvpn=",
                    "admin/network/vpn/zerotier",
                ):
                    zerotier_settings_html = await hass.async_add_executor_job(
                        router.get,
                        zerotier_settings_path,
                        True,
                    )
                    if not zerotier_settings_html:
                        continue

                    zerotier_settings = parse_zerotier_settings(zerotier_settings_html)
                    if zerotier_settings:
                        vpn_settings.update(zerotier_settings)
                        break
        ...
        configured_vpn_protocol = _entry_value(vpn_settings, "protocol")
        ...
        vpn_status_confirms_enabled = False
        for vpn_status_path in _VPN_STATUS_PATHS:
            ...
            if (
                isinstance(configured_vpn_protocol, str)
                and configured_vpn_protocol in _VPN_STATUS_PATH_PROTOCOLS.get(vpn_status_path, frozenset())
                and _vpn_status_indicates_connected(candidate_html)
            ):
                vpn_status_confirms_enabled = True
        ...
        if vpn_status_confirms_enabled and vpn_settings and _entry_value(vpn_settings, "enabled") is None:
            vpn_settings["enabled"] = {"value": True}
            data[MODULE_VPN_SETTINGS] = vpn_settings
