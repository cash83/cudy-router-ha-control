...
def parse_zerotier_settings(input_html: str) -> dict[str, Any]:
    """Parse the dedicated ZeroTier VPN configuration page."""
    soup = _soup(input_html)

    data: dict[str, Any] = {}

    enabled = _hidden_bool(soup, "cbid.zerotier.client.enabled")
    if enabled is not None:
        data["enabled"] = {"value": enabled}

    site_to_site = _hidden_bool(soup, "cbid.zerotier.client.s2s")
    if site_to_site is not None:
        data["site_to_site"] = {"value": site_to_site}

    for key, field_name in (
        ("default_rule", "cbid.zerotier.client.filter"),
        ("client_access", "cbid.zerotier.client.access"),
        ("vpn_policy", "cbid.zerotier.client.policy"),
    ):
        entry = _select_entry(soup, field_name)
        if entry is not None:
            data[key] = entry
...
def parse_wireless_settings(
    ...
    if smart_connect:
        ...
        band_2g_field = lambda suffix: [f"{band_2g_prefix}_{suffix}", f"{band_2g_prefix}.{suffix}"]
        band_5g_field = lambda suffix: [f"{band_5g_prefix}_{suffix}", f"{band_5g_prefix}.{suffix}"]
    else:
        enabled = None
        hidden = None
        isolate = None
        ...
        band_2g_field = lambda suffix: [f"{band_2g_prefix}.{suffix}"]
        band_5g_field = lambda suffix: [f"{band_5g_prefix}.{suffix}"]
    ...
    for key, entry in (
        ("wifi_2g_mode", _first_select_entry(active_soup, band_2g_field("hwmode"))),
        ("wifi_2g_channel_width", _first_select_entry(active_soup, band_2g_field("htbw"))),
        ("wifi_2g_channel", _first_select_entry(active_soup, band_2g_field("channel"))),
        ("wifi_2g_tx_power", _first_select_entry(active_soup, band_2g_field("txpower"))),
        ("wifi_5g_mode", _first_select_entry(active_soup, band_5g_field("hwmode"))),
        (
        ...
        (
            ...
            _first_select_entry(
                active_soup,
                band_5g_field("channel4")
                + band_5g_field("channel3")
                + band_5g_field("channel2")
                + band_5g_field("channel"),
            ),
        ),
        ("wifi_5g_tx_power", _first_select_entry(active_soup, band_5g_field("txpower"))),
    ):
        if entry is not None:
            data[key] = entry
