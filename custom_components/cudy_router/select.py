...
SETTINGS_REFRESH_DELAY_SECONDS = 8
...
class CudyRouterSettingSelect(
    ...
    def current_option(self) -> str | None:
        ...
        value = self._optimistic_value if self._optimistic_value is not None else setting.get("value")
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        if result[0] in (200, 302):
            self._optimistic_value = selected_value
            self.async_write_ha_state()
            try:
                await asyncio.sleep(SETTINGS_REFRESH_DELAY_SECONDS)
